"""
Same edge as backtest: passive at level -> retest -> bounce -> BOS -> aggressive -> LONG.
TP at COB resistance (0.5 pt below). Exit: SL, reversal BOS, or TP.
State: in_position, entry_price, entry_bar_idx, running_low, key_levels_at_entry, sl_price.
"""
import numpy as np
import pandas as pd
...
def _aggressive_accumulation_bars(bars: pd.DataFrame, entry_bar_idx: int, bar_sec: float,
                                  agg_win_sec: int, min_volume: int) -> bool:
...
def process_bar(bars: pd.DataFrame, current_bar_idx: int, params: dict, state: dict,
    ...
    if state.get("in_position"):
        ...
        if p <= sl_price:
            ...
            return "TAKE_PROFIT", state  # exit (SL)
    ...
    while i <= current_bar_idx - passive_lookback_bars - bos_lookback * 2 - bounce_bars - 1:
        ...
        for t in range(i, min(i + bos_search_bars, n - bos_lookback * 2 - bounce_bars - 1)):
            ...
            if entry_price < acc_level - kl_pts * POINT:
                continue
            if not _aggressive_accumulation_bars(bars, entry_bar_idx, bar_sec, agg_win, agg_vol):
                continue
